            ...
            validRaces.forEach((race, index) => {
                ...
                resultData.forEach(result => {
                    ...
                    if (pilot) {
                        ...
                        // ★デバッグ用に追加
                        console.log(`[Debug] raceResultsに行を追加: ${newRow.length}列, HEAT: ${raceName}, Pilot: ${pilot.Name}`);
...
async function updateGoogleSheet(raceResults, lapsToDo) {
    try {
        ...
        // ★★★ ヘッダーの列数を確認するデバッグコード ★★★
        console.log(`[DEBUG] Number of headers: ${raceResultHeaders.length}`);
        ...
        /* if (raceResults.length > 0) {
            // ★★★ 各行の列数をチェックするデバッグコード ★★★
            raceResults.forEach((row, rowIndex) => {
                if (row.length !== 43) {
                    console.log(`[DEBUG] 列数異常! Row ${rowIndex} (HEAT: ${row[1]}) has ${row.length} columns. Expected 43.`);
                }
            });
            requests.push({
                updateCells: {
        ...
        if (requests.length > 0) {
            // ★★★ デバッグ用にrequestsオブジェクトをコンソールに出力 ★★★
            console.log('--- DEBUG START: requests object for updateGoogleSheet ---');
            console.log(JSON.stringify(requests, null, 2));
            console.log('--- DEBUG END: requests object for updateGoogleSheet ---');
